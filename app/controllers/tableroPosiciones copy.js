...
const options = {
    page: 1,
    limit: 50,
    select: '_id fecha liga equipoLocal equipoVisitante posicionLocal posicionVisita cornersHechosLocal cornersRecibidosLocal cornersHechosVisita cornersRecibidosVisita cornersHechoTotalesLocalVisita cornersProbabilidadMas6 golesHechosLocal golesRecibidosLocal golesHechosVisita golesRecibidosVisita golesHechoTotalesLocalVisita golesProbabilidadMas6 idAwayTeam idHomeTeam'
}

exports.insertDataMasivo = async(req, res, next) => {
    const conn = conectionManager(req);
    try {
        const TableroPosiciones = getModel(conn, consta.SchemaName.tableroPosiciones);
        const data = req.body;

        await TableroPosiciones.deleteMany({});
        // .deleteMany({})
        const result = await TableroPosiciones.insertMany(data)

        const cacheKey = consta.cacheController.tableroPosiciones.maestros;
...
exports.getMaestros = async(req, res, next) => {
    ...
    try {
        ...
        const [strLeague] = await Promise.all([
            TableroPosiciones.distinct("liga", query)
        ])
...
exports.getCriterio = async(req, res, next) => {
    ...
    try {
        ...
        await TableroPosiciones.paginate(filtro, options, (err, result) => {
            if (err) throw err;
