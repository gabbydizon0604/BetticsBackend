...
exports.insertDataMasivo = async(req, res, next) => {
    ...
    try {
        const EventosLiga = getModel(conn, consta.SchemaName.eventosLigaProb);
        const data = req.body;
...
exports.getCriterio = async(req, res, next) => {
    ...
    try {
        ...
        const [partidosLosDosEquipos, ultimos6Home, ultimos6Away, soloLocal, soloVisita] = await Promise.all([
            EventosLiga.find(filtroInit).select("strSeason dateEvent idAwayTeam idHomeTeam strHomeTeam strAwayTeam intHomeScore intAwayScore cornerKicksHome cornerKicksAway shotsonGoalHome shotsonGoalAway totalCardsHome totalCardsAway")
            .sort({ dateEvent: -1 }),
            // Ultimos 6 partidos
            EventosLiga.find(filtro1).select("dateEvent idAwayTeam idHomeTeam strHomeTeam strAwayTeam intHomeScore intAwayScore cornerKicksHome cornerKicksAway shotsonGoalHome shotsonGoalAway totalCardsHome totalCardsAway")
            .sort({ dateEvent: -1 }),
            // Ultimos 6 partidos
            EventosLiga.find(filtro2).select("dateEvent idAwayTeam idHomeTeam strHomeTeam strAwayTeam intHomeScore intAwayScore cornerKicksHome cornerKicksAway shotsonGoalHome shotsonGoalAway totalCardsHome totalCardsAway")
            .sort({ dateEvent: -1 }),
            EventosLiga.find(filtro3).select("dateEvent idAwayTeam idHomeTeam strHomeTeam strAwayTeam intHomeScore intAwayScore cornerKicksHome cornerKicksAway shotsonGoalHome shotsonGoalAway totalCardsHome totalCardsAway")
            .sort({ dateEvent: -1 }),
            EventosLiga.find(filtro4).select("dateEvent idAwayTeam idHomeTeam strHomeTeam strAwayTeam intHomeScore intAwayScore cornerKicksHome cornerKicksAway shotsonGoalHome shotsonGoalAway totalCardsHome totalCardsAway")
            .sort({ dateEvent: -1 })
        ])
